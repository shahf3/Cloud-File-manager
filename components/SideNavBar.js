...
function SideNavBar() {
  ...
  const handleUploadFileClick = () => {
    const modal = document.getElementById('upload_file');
    if (modal) modal.showModal();
  };
  ...
  return (
    session && (
      <div className="w-[200px] bg-white h-screen sticky top-0 z-10 shadow-blue-200 shadow-md p-5">
        ...
        <div className="mt-7">
          {menu && menu.list && Array.isArray(menu.list) ? (
            menu.list.map((item, index) => (
              <h2
                key={item.id}
                onClick={() => onMenuClick(item, index)}
                className={`flex gap-2 items-center p-2 mt-3 text-gray-500 rounded-md cursor-pointer hover:bg-blue-500 hover:text-white ${activeIndex === index ? "bg-blue-500 text-white" : null}`}
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
        ...
        <dialog id="upload_file" className="modal">
          <UploadFileModal closeModal={() => document.getElementById('upload_file').close()} />
        </dialog>
      </div>
    )
  );
